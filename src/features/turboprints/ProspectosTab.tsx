...
function ProspectoForm({ prospecto, onClose }: { prospecto?: Prospecto; onClose: () => void }) {
  ...
  return (
    <Modal title={prospecto ? 'Editar prospecto' : 'Nuevo prospecto'} modalClassName="tp-modal" onClose={onClose}>
      <div className="field">
        <label>Nombre</label>
      ...
      <div className="field">
        <label>Interés</label>
        <input value={interes} onChange={(e) => setInteres(e.target.value)} />
      </div>
      <div className="field">
        <label>Fecha</label>
